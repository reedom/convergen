...
-   [x] **1.1. Create `pkg/builder/handler.go`:** This new file will house the core handler interface.
-   [x] **1.2. Define `AssignmentHandler` interface:** The interface will be defined as follows:
    ```go
    ...
    ```
-   [x] **1.3. Create a base handler struct:** Implement a small, embeddable struct that provides a default implementation for `SetNext` to reduce boilerplate in concrete handlers.
...
-   [x] **4.1. Create `pkg/builder/handler_test.go`:**
