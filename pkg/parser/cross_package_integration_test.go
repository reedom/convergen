...
`
	require.NoError(t, os.WriteFile(userModelFile, []byte(userModel), 0o600))
...
`
	require.NoError(t, os.WriteFile(userDTOFile, []byte(userDTO), 0o600))

	// Create go.mod files
	modelsModFile := filepath.Join(modelsDir, "go.mod")
	modelsModContent := "module models\n\ngo 1.21\n"
	require.NoError(t, os.WriteFile(modelsModFile, []byte(modelsModContent), 0o600))

	dtoModFile := filepath.Join(dtoDir, "go.mod")
	dtoModContent := "module dto\n\ngo 1.21\n"
	require.NoError(t, os.WriteFile(dtoModFile, []byte(dtoModContent), 0o600))
